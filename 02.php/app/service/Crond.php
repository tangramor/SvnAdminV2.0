...
class Crond extends Base
{
    ...
    public function GetRepList()
    {
        ...
        if ($this->userRoleId == 1) {
            $list = array_merge([[
                'rep_key' => '-1',
                'rep_name' => $this->L->translate('all_repos') //'所有仓库'
            ]], $list);
        }
    ...
    public function CreateCrontab()
    {
        if (!isset($this->payload['cycle'])) {
            return message(200, 0, $this->L->translate('param_cycle_not_exist'));    //'参数[cycle]不存在'
        }
        ...
        switch ($cycle['cycle_type']) {
            case 'minute': //每分钟
                $code = '* * * * *';
                $cycle['cycle_desc'] = $this->L->translate('exec_per_minute');    //"每分钟执行一次"
                break;
            case 'minute_n': //每隔N分钟
                $code = sprintf("*/%s * * * *", $cycle['minute']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_n_minutes'), $cycle['minute']);    //"每隔%s分钟执行一次"
                break;
            case 'hour': //每小时
                $code = sprintf("%s * * * *", $cycle['minute']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_hour_n_minute'), $cycle['minute']);    //"每小时-第%s分钟执行一次"
                break;
            case 'hour_n': //每隔N小时
                $code = sprintf("%s */%s * * *", $cycle['minute'], $cycle['hour']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_n_hours_n_minute'), $cycle['hour'], $cycle['minute']);    //"每隔%s小时-第%s分钟执行一次"
                break;
            case 'day': //每天
                $code = sprintf("%s %s * * *", $cycle['minute'], $cycle['hour']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_day_n_hour_n_minute'), $cycle['hour'], $cycle['minute']);    //"每天-%s点%s分执行一次"
                break;
            case 'day_n': //每隔N天
                $code = sprintf("%s %s */%s * *", $cycle['minute'], $cycle['hour'], $cycle['day']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_n_days_n_hour_n_minute'), $cycle['day'], $cycle['hour'], $cycle['minute']);    //"每隔%s天-%s点%s分执行一次"
                break;
            case 'week': //每周
                $code = sprintf("%s %s * * %s", $cycle['minute'], $cycle['hour'], $cycle['week']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_weekday_n_hour_n_minute'), $cycle['week'], $cycle['hour'], $cycle['minute']);    //"每周%s-%s点%s分执行一次"
                break;
            case 'month': //每月
                $code = sprintf("%s %s %s * *", $cycle['minute'], $cycle['hour'], $cycle['day']);
                $cycle['cycle_desc'] = sprintf($this->L->translate('exec_per_monthday_n_hour_n_minute'), $cycle['day'], $cycle['hour'], $cycle['minute']);    //"每月%s日-%s点%s分执行一次"
                break;
            default:
                break;
        }
        ...
        return message(200, 1, $this->L->translate('success'));
    }
    ...
    public function UpdCrontabStatus()
    {
        ...
        if (empty($result)) {
            return message(200, 0, $this->L->translate('schedule_task_not_exist'));    //'任务计划不存在'
        }
    ...
    public function GetCrontabLog()
    {
        ...
        if (file_exists($this->configSvn['crond_base_path'] . $sign . '.log')) {
            ...
            return message(200, 1, $this->L->translate('success'), [    //‘成功'
                'log_path' => $this->L->translate('log_path_not_exist'),   //'未生成'
                'log_con' => ''
            ]);
        }
    }
    ...
    public function GetCronStatus()
    {
        ...
        if (empty($resultCrond['result'])) {
            ...
            if (empty($resultCron)) {
                return message(200, 0, $this->L->translate('cron_service_not_start')); //'cron或crond服务未启动'
            }
        }

        $resultCron = funShellExec('ps aux | grep -v grep | grep atd');
        if (empty($resultCron)) {
            return message(200, 0, $this->L->translate('atd_service_not_start'));  //'atd服务未启动'
        }
