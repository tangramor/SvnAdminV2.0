...
class Secondpri extends Base
{
    ...
    public function UpdSecondpri()
    {
        ...
        if (!in_array($this->payload['type'], [0, 1])) {
            return message(200, 0, $this->L->translate('unsupport_type')); //'不支持的类型'
        }
    ...
    public function CreateSecondpriObject()
    {
        ...
        if (!in_array($this->payload['objectType'], ['user', 'group', 'aliase'])) {
            return message(200, 0, $this->L->translate('unpermitted_object_type'));    //'不允许的对象类型'
        }
        ...
        if (empty($userName)) {
            return message(200, 0, $this->L->translate('permission_path_record_not_exist'));   //'权限路径记录不存在'
        }

        if ($this->payload['objectType'] == 'user' && $this->payload['objectName'] == $userName) {
            return message(200, 0, $this->L->translate('manageable_object_cannot_include_itself'));    //'可管理对象不能包括本身'
        }
        ...
        if (!empty($result)) {
            return message(200, 0, $this->L->translate('object_already_exists'));    //'对象已存在'
        }
