...
class Apache extends Base
{
    ...
    private function UpdConfSvn($prefix = '')
    {
        ...
        if ($dataSource['user_source'] == 'httpPasswd') {
            ...
            if (!is_readable($templeteSubversionPath)) {
                return message(200, 0, sprintf($this->L->translate('file_not_exist_or_not_readable'), $templeteSubversionPath));   //'文件[%s]不可读或不存在'
            }
            ...
            if (!file_exists($subversionPath)) {
                ...
                if (!file_exists($subversionPath)) {
                    ...
                    if (!file_exists($subversionPath)) {
                        return message(200, 0, sprintf($this->L->translate('cannot_create_file'), $subversionPath));   //'无法创建文件[%s]'
                    }
                }
            }
            ...
            if (!is_writeable($subversionPath)) {
                funFilePutContents($subversionPath, $subversion, true);
                if (!is_writeable($subversionPath)) {
                    return message(200, 0, sprintf($this->L->translate('cannot_write_file'), $subversionPath));    //'无法写入文件[%s]'
                }
            }
    ...
    public function UpdSubversionDisable()
    {
        ...
        if (file_exists($this->configSvn['apache_subversion_file'])) {
            return message(200, 0, sprintf($this->L->translate('cannot_remove_file'), $this->configSvn['apache_subversion_file']));    //"无法移除文件[%s]"
        }
    ...
    public function CreateUserStr($username, $password)
    {
        ...
        return message(200, 1, $this->L->translate('success'), $result['result']);
    }
    ...
    public function UpdHttpPort()
    {
        if ($this->enableCheckout == 'svn') {
            return message(200, 0, $this->L->translate('need_to_be_http_protocol_for_modification'));  //'需要切换为http协议检出状态才可修改'
        }
    ...
    public function UpdHttpPrefix()
    {
        ...
        if (substr($this->payload['prefix'], 0, 1) != '/') {
            return message(200, 0, $this->L->translate('prefix_need_slash'));  //'前缀要携带/'
        }
