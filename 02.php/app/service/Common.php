...
class Common extends Base
{
    ...
    public function Login()
    {
        ...
        if ($verifyOption['enable'] == true) {
            ...
            if (empty($endTime)) {
                ...
                return message(200, 0, $this->L->translate('login_failed_by_wrong_captcha'), $endTime); //'登录失败[验证码错误]'
            }
            if ($endTime == 0) {
                return message(200, 0, $this->L->translate('login_failed_by_invalid_captcha'));    //'登陆失败[验证码失效]'
            }
            if ($endTime < time()) {
                return message(200, 0, $this->L->translate('login_failed_by_expired_captcha'));    //'登陆失败[验证码过期]'
            }
        }
        ...
        if ($userRole == 1) {
            ...
            if (empty($result)) {
                return message(200, 0, $this->L->translate('login_failed_by_wrong_account_or_password'));    //'登录失败[账号或密码错误]'
            }
            ...
            if ($dataSource['user_source'] == 'ldap') {
                ...
                if (empty($result)) {
                    return message(200, 0, $this->L->translate('login_failed_by_nosync_ldap_account'));    //'登录失败[ldap账户未同步]'
                }

                if (!$this->ServiceLdap->LdapUserLogin($userName, $userPass)) {
                    return message(200, 0, $this->L->translate('login_failed_by_ldap_auth_failed'));    //'登录失败[ldap账户认证失败]'
                }
                ...
                if (strstr($userName, '|')) {
                    return message(200, 0, $this->L->translate('login_failed_by_ldap_username_invalid_format'));   //'登录失败[ldap账户名不合法]'
                }
            } else {
                if ($this->enableCheckout == 'svn') {
                    ...
                    if ($result['svn_user_status'] == 0) {
                        return message(200, 0, $this->L->translate('login_failed_by_session_expired'));    //'登录失败[用户已过期]'
                    }
                } else {
                    ...
                    if (empty($result)) {
                        return message(200, 0, $this->L->translate('login_failed_by_user_not_synced'));    //'登录失败[用户未同步]'
                    }
                    if ($result['svn_user_status'] == 0) {
                        return message(200, 0, $this->L->translate('login_failed_by_session_expired'));    //'登录失败[用户已过期]'
                    }
        ...
        //邮件
        $this->Mail->SendMail('Common/Login', $this->L->translate('notice_about_user_login'),  //'用户登录成功通知'
             $this->L->translate('user_account')   //'账号：'
             . $userName . ' ' . 
             $this->L->translate('ip_address')    //'IP地址：'
             . funGetCip() . ' ' . 
             $this->L->translate('time')   //'时间：'
             . date('Y-m-d H:i:s'));

        $info = $this->GetDynamicRouting($userName, $userRole);
        return message(200, 1, $this->L->translate('login_success'), [ //'登陆成功'
            'token' => $token,
            'user_name' => $userName,
    ...
    public function Logout()
    {
        ...
        //退出
        return message(200, 1, $this->L->translate('logout_success')); //'退出登录成功'
    }
    ...
    public function GetVerifyCode()
    {
        ...
        if (empty($codeId)) {
            return message(200, 0, $this->L->translate('cannot_write_into_db_need_permission_for_sqlite'));    //'无法写入数据库，如果为 SQLite，请为数据库文件及上级目录授权'
        }
